"""Model evaluation on the test set with metrics and visualisations for N-HiTS.
...
def _plot_forecast_vs_actual(
    ...
    ax.set_title("N-HiTS — Predicted vs Actual Electricity Demand (test set, scaled)")
    ax.set_xlabel("Hour")
...
def _plot_error_by_horizon(
    ...
    ax.set_title("MAE by Forecast Horizon Step (N-HiTS)")
...
def _plot_training_history(
    ...
    ax.set_title("Training History (N-HiTS)")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.legend()
    ax.grid(True, alpha=0.3)
...
def evaluate(
    ...
    ----------
    model:
        Trained ``NHiTSModel`` in eval mode.
    test_loader:
        Test DataLoader.
    cfg:
        ``NHiTSConfig`` instance (used for ``output_dir``).
    device:
        Inference device.
    history:
